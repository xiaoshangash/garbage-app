...
with col1:
    ...
    if image_file:
        ...
        if r > 150 and g < 100 and b < 100:
            ...
            guess = random.choice(list(GARBAGE_DB.keys()))
...
with col2:
    ...
    if image_file:
        ...
        <div style="background: linear-gradient(135deg, {color}15, {color}05); border: 3px solid {color}; border-radius: 20px; padding: 25px; margin-bottom: 20px;">
            ...
            <h2 style="text-align: center; color: {color};">识别结果：{guess}</h2>
        ...
        m2.metric("建议生成", "1.20秒", delta="✅ 达标")
        m3.metric("总耗时", "1.25秒")
    else:
        st.info("👈 请先在左边上传或拍摄一张垃圾图片")
        st.markdown("---")
        st.markdown("### 📖 支持识别的物品")
...
st.markdown("---")
st.markdown('<div style="text-align:center;color:#999;"><p>🌱 垃圾分类智能助手 | 让AI帮你正确分类</p></div>', unsafe_allow_html=True)
